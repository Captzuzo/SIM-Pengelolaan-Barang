    ...
    @if (!empty($data['penjualans']) && $data['penjualans']->count())
        ...
        <div class="mt-6 grid gap-4 sm:grid-cols-3">
            <div class="p-4 bg-green-50 border border-green-200 rounded-lg shadow-sm">
                ...
                <p class="mt-1 text-xl font-bold text-green-900">Rp {{ number_format($data['total_penjualan'], 0, ',', '.') }}</p>
            </div>
            <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg shadow-sm">
                ...
                <p class="mt-1 text-xl font-bold text-blue-900">Rp {{ number_format($data['total_modal'], 0, ',', '.') }}</p>
            </div>
            <div class="p-4 bg-yellow-50 border border-yellow-200 rounded-lg shadow-sm">
                ...
                <p class="mt-1 text-xl font-bold text-yellow-900">Rp {{ number_format($data['total_laba'], 0, ',', '.') }}</p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse">
                    ...
                    <tbody>
                        ...
                        @forelse ($data['penjualans'] as $item)
                            @foreach ($item->detail as $detail)
                                ...
                                <tr class="text-center text-sm">
                                    ...
                                    @if ($loop->first)
                                        <td class="border px-2 py-1 text-green-600 font-semibold" rowspan="{{ $item->detail->count() }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="border px-2 py-2 text-center text-dark-500">Tidak ada data</td>
                            </tr>
                        @endforelse

                        {{-- Total Keseluruhan --}}
                        <tr class="bg-dark-100 font-bold text-center">
                            <td colspan="3" class="border px-2 py-1 text-right">Total Keseluruhan</td>
